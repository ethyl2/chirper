...
<template>
  <div class="p-6 flex space-x-2 rounded shadow bg-white" :style="`border: 2px solid ${ chirp.color};`">
    ...
    <div class="flex-1">
      ...
      <form v-if="editing" @submit.prevent="form.put(route('chirps.update', chirp.id), { onSuccess: editing = false })">
        ...
        <input type="text" v-model="form.color" placeholder="border color"
          class="mt-4 border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm" />
